...
from nearest_neighbours import nearest_neighbours_evaluate, calculate_initial_distances, update_distances, nearest_neighbours_evaluate_2, find_initial_nearest_neighbour, find_nearest_neighbour_
from data import generate_data
from plotter import plot_sample_complexity


def evaluate_1nn():
    n_max, m_max, num_runs = 100, 20000, 10
    ...
    for i in tqdm(range(1, num_runs + 1)):
        ...
        dev_x, dev_y = generate_data(50000, n_max)
        ...
        for n in tqdm(range(1, n_max + 1)):
            ...
            for m in range(1, m_max + 1):
                ...
                if (n < 2 and nn_count < 3) or (
                    ...
                    if m == last_m:
                        min, argmin = find_initial_nearest_neighbour(distances, m)
                    else:
                        min, argmin = find_nearest_neighbour_(distances, m, min, argmin)
                    nn_error = nearest_neighbours_evaluate(argmin, current_y, dev_y)
...
if __name__ == '__main__':
    evaluate_1nn()
    n_max, m_max, num_runs = 100, 8000, 50
    ...
    # Matrix to indicate where NN makes 0 errors (as opposed to just skipping a particular (m, n) pair).
    nn_changes = np.zeros_like(perceptron_errors)

    for i in tqdm(range(1, num_runs + 1)):
        train_x, train_y = generate_data(m_max, n_max)
        dev_x, dev_y = generate_data(2000, n_max)
        distances = calculate_initial_distances(dev_x, train_x)
        last_m, nn_changes_temp, nn_errors_temp = 0, np.zeros_like(nn_changes), np.zeros_like(nn_errors)
        for n in tqdm(range(1, n_max + 1)):
            current_dev_x = dev_x[:, :n]
            winnow_dev_x, winnow_dev_y = np.copy(dev_x), np.copy(dev_y)
            winnow_dev_x[winnow_dev_x == -1] = 0
