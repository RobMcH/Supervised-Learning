...
def kernelise_symmetric(x_i, kernel_function, kernel_parameter):
    ...
    resulting kernel and should be used whenever possible (as opposed to the kernelise method).
    """
    length_i = len(x_i)
...
def train_binary_kernel_perceptron(train_y, kernel_matrix, max_iterations=100):
    ...
    # Use sparsity to ignore any parts of the dot product where alpha is 0.
    mask = alphas != 0
    while True:
        error = 0
        for i in range(train_y.size):
            ...
            y = train_y[i]
            if y_hat != y:
                # Update weights and increase error counter if prediction was wrong. Update mask.
                alphas[i] += y
                mask[i] = alphas[i] != 0
                error += 1
        if error < last_error:
...
def train_ova_kernel_perceptron(train_y, kernel_matrix, max_iterations):
    ...
    # Train num_classes binary classifiers.
...
def train_kernel_perceptron(train_y, kernel_matrix, max_iterations=100):
    ...
    while True:
        ...
        for i in range(train_y.size):
            ...
            # Increase error counter and update weights. When prediction is correct the updates cancel out.
            alphas[y_hat, i] -= 1
...
def train_ovo_kernel_perceptron_(train_y, kernel_matrix, class_combinations, max_alpha_len):
    # Not used in the assignment. Showed poor performance.
